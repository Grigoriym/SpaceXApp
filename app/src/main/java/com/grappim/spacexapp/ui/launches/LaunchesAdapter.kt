...
import kotlinx.android.synthetic.main.layout_launches_item.view.tvLaunchesFlightNumber
import kotlinx.android.synthetic.main.layout_launches_item.view.tvLaunchesMissionName
import java.util.*
...
class LaunchesAdapter(
  val onClick: (LaunchModel) -> Unit
    ...
    private fun getFilteredResults(constraint: String): List<LaunchModel> {
        ...
        for (i in items) {
            if (i.missionName != null && i.missionName.toLowerCase(Locale.ROOT)
                    .contains(constraint)
            ) {
                results.add(i)
            }
